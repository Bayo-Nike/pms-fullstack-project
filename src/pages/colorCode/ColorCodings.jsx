...
export default function ColorCodings() {
    ...
    const filteredColorCodings = useMemo(() => {
        return colorCodes.filter(c => (c.fiscalYear || "").toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <div className="w-full space-y-4 animate-fadeIn px-2 pb-10 relative">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                ...
                {can('CAN_MANAGE_MODULES') && (
                    <button onClick={() => navigate('/colorCode/CreateColorCoding/create')} className="bg-[#FBAF1E] text-white px-5 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg shadow-amber-100 hover:bg-[#e09a15] transition-all uppercase tracking-widest">
            ...
            <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm flex items-center">
                <div className="relative max-w-sm w-full">
                    ...
                    <input type="text" placeholder="Search Color Code ..." className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs outline-none focus:border-[#0284C7] transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
            </div>
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {loading ? (
                            ...
                            <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 bg-sky-50 text-[#0284C7] rounded-lg flex items-center justify-center group-hover:bg-[#0284C7] group-hover:text-white transition-all"><CorporateFare style={{ fontSize: 18 }} /></div>
                                        <span className="text-sm font-semibold text-slate-700">{c.target}</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                <div className="flex items-center gap-3">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => navigate(`/colorCode/ColorCodings/edit/${c.id}`)} className="p-1.5 text-slate-400 hover:text-[#0284C7] hover:bg-sky-50 rounded-md transition-all"><Edit style={{ fontSize: 18 }} /></button>
