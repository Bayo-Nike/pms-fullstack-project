...
export default function ContractorReport({ data = [], loading, onRefresh }) {
  const [globalFilter, setGlobalFilter] = useState("");
  const [sorting, setSorting] = useState([]);

  const columns = useMemo(() => [
    { accessorKey: "sno", header: "S/No" },
    { accessorKey: "contractorName", header: "Contractor Name" },
    { accessorKey: "status", header: "Status" },
    { accessorKey: "document", header: "File Name" },
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-[400px] text-slate-500">
        Synchronizing Contractor Report Engine...
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border shadow-sm">
      ...
      <div className="p-5 border-b flex justify-between items-center">
        <div>
          <h1 className="text-lg font-bold">Contractor Master Report</h1>
          <p className="text-xs text-slate-400">Enterprise Contractors Management</p>
        </div>
        ...
        <div className="flex gap-3">
          ...
          {/* Export */}
          <button
            onClick={() => exportToExcel(columns, data, "Contractor_Report")}
          ...
          <button
            onClick={() => exportToPDF(columns, data, "Contractor_Report")}
