...
export default function ViewTarget() {
  ...
  const { isLoaded } = useJsApiLoader({
    ...
    googleMapsApiKey: "AIzaSyDLL2kzXcOFh6mA5J9A45Nl9sfB5COmHFw"
  });
  ...
  // --- UPDATED MAP CLICK LOGIC FOR GOOGLE ---
  const handleMapClick = useCallback((e) => {
    if (!canSendColorCodingAchievement) return;
  ...
  // --- UPDATED MARKER DRAG LOGIC FOR GOOGLE ---
  const handleMarkerDrag = (index, e) => {
    const lat = e.latLng.lat();
    const lng = e.latLng.lng();
    const updated = [...achievementForm.locations];
                ...
                <div className="h-[400px] w-full rounded-3xl overflow-hidden border-4 border-slate-50 shadow-inner z-0 relative">
                  {isLoaded ? (
                    <GoogleMap
                      ...
                      center={achievementForm.locations[0]?.latitude ? 
                        { lat: parseFloat(achievementForm.locations[0].latitude), lng: parseFloat(achievementForm.locations[0].longitude) } : 
                        defaultCenter
                      }
                      zoom={12}
                      ...
                      {achievementForm.locations.map((loc, idx) => (
                        loc.latitude && (
                          <Marker
                            ...
                            draggable={canSendColorCodingAchievement}
                            onDragEnd={(e) => handleMarkerDrag(idx, e)}
                          />
                        )
                      ))}
                    </GoogleMap>
                  ) : (
                    <div className="flex items-center justify-center h-full bg-slate-50 animate-pulse text-xs text-slate-400 uppercase font-bold">
                      Initializing Maps...
                    </div>
                  )}
                </div>
