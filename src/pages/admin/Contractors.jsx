...
export default function Contractors() {
    ...
    const fetchContractors = async () => {
        try {
            ...
            setContractors(res.data?.data || res.data || res);
        } catch (err) {
    ...
    const filteredContractors = useMemo(() => {
        return contractors.filter(c => (c.contractorName || "").toLowerCase().includes(searchTerm.toLowerCase()));
    }, [contractors, searchTerm]);

    const paginatedItems = filteredContractors.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
        ...
        <div className="w-full space-y-4 animate-fadeIn px-2 pb-10 relative">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                ...
                {can('CAN_MANAGE_CONTRACTOR') && (
                    <button onClick={() => navigate('/contractors/create')} className="bg-[#FBAF1E] text-white px-5 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg shadow-amber-100 hover:bg-[#e09a15] transition-all uppercase tracking-widest">
                        <Add style={{ fontSize: 18 }} /> Register Contractor
                    </button>
                )}
            </div>
            ...
            <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm flex items-center">
                <div className="relative max-w-sm w-full">
                    ...
                    <input type="text" placeholder="Search Contractor..." className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs outline-none focus:border-[#0284C7] transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
            </div>
                ...
                <table className="w-full text-left">
                    <thead className="bg-slate-50/50 border-b border-slate-100 text-slate-400 text-[9px] font-bold uppercase tracking-widest">
                        <tr>
                            ...
                            <th className="px-6 py-4">Contractor Category</th>
                            ...
                            <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm font-semibold text-slate-700">{c.contractorName}</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 bg-sky-50 text-[#0284C7] rounded-lg flex items-center justify-center group-hover:bg-[#0284C7] group-hover:text-white transition-all"><DateRange style={{ fontSize: 18 }} /></div>
                                        <span className="text-sm font-semibold text-slate-700">{c.licenseExpiryDate}</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4">
                                    {c.document ? (
                                        <a href={`http://localhost:8080/api/admin/contractors/download/${c.document}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-[#0284C7] hover:text-[#016da3] transition-colors">
