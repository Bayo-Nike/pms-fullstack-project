...
export default function Contractors() {
    ...
    const executeDelete = async () => {
        ...
        try {
            ...
            showAlert('success', `contractor "${name}" has been removed.`);
            fetchDivisions();
        } catch (err) {
            const errorMsg = err.response?.data?.message || "Deletion failed: Ensure this contractor has no sub-divisions or active users.";
            showAlert('error', errorMsg);
        }
    };

    const filteredContractors = useMemo(() => {
        return contractors.filter(d =>
    ...
    const paginatedContractors = useMemo(() => {
        ...
        return filteredContractors.slice(firstPageIndex, lastPageIndex);
    ...
    return (
        <div className="w-full space-y-4 animate-fadeIn px-2 pb-10 relative">
            ...
            {deleteConfig.show && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full mx-4 border border-slate-100 text-center">
                        ...
                        <h3 className="text-lg font-bold text-slate-800 uppercase tracking-tight">Delete Contractor</h3>
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                <div>
                    <h1 className="text-base font-bold text-slate-900">Contractors</h1>
                    <p className="text-[10px] text-slate-400 uppercase tracking-wider">Organizational Setup</p>
                </div>
                {can('CAN_MANAGE_MODULES') && (
                    <button onClick={() => navigate('/admin/contractors/create')} className="bg-[#FBAF1E] text-white px-5 py-2 rounded-lg font-bold text-xs flex items-center gap-2 shadow-sm transition-transform active:scale-95 uppercase tracking-widest">
                        <Add style={{ fontSize: 18 }} /> Create Contractor
                    </button>
                )}
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50 border-b border-slate-100 text-slate-400 text-[9px] font-bold uppercase tracking-widest">
                        <tr>
                            <th className="px-6 py-3">Contractor Name</th>
                ...
                <div className="px-6 py-4 bg-slate-50/20 border-t border-slate-100 flex items-center justify-between">
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        Showing {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredContractors.length)} of {filteredContractors.length}
                    </div>
