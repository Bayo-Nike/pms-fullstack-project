...
export default function CreateEmployee() {
    ...
    // 1. FILTER LOGIC: Filter positions based on selected divisionId
    const filteredPositions = useMemo(() => {
        if (!formData.divisionId) return [];
        return positions.filter(p => String(p.divisionId) === String(formData.divisionId));
    }, [formData.divisionId, positions]);
    ...
    const handleInputChange = (e) => {
        ...
        // 2. RESET LOGIC: If division changes, clear the selected position
        if (name === 'divisionId') {
            setFormData(prev => ({
                ...prev,
                divisionId: value,
            ...
            {showConfirm && (
                <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4 border border-slate-100 text-center">
                        <HelpOutline className="text-[#0284C7] mb-4 mx-auto" style={{ fontSize: 56 }} />
                        <h3 className="text-xl font-bold text-slate-800 uppercase tracking-tight">Confirm Save</h3>
                        <p className="text-sm text-slate-500 mt-2">Save changes for <b>{formData.fullName}</b>?</p>
                        <div className="flex gap-3 mt-8">
                            <button onClick={() => setShowConfirm(false)} className="flex-1 px-4 py-3 rounded-xl border text-xs font-bold uppercase tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
                            <button onClick={executeSave} className="flex-1 px-4 py-3 rounded-xl bg-[#0284C7] text-white font-bold text-xs uppercase tracking-widest shadow-lg shadow-sky-100 hover:bg-[#016da3] transition-all">Confirm</button>
                        </div>
                ...
                <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden h-fit">
                    ...
                    <div className="p-6 space-y-5">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" style={{ fontSize: 18 }} />
                                <input name="email" type="email" value={formData.email} onChange={handleInputChange} className="w-full pl-10 pr-4 py-3 text-sm font-semibold bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#0284C7]" placeholder="email@company.com" />
                            </div>
                        </div>
                        <div>
                            <label className="text-[9px] font-bold uppercase text-slate-400 tracking-[0.2em] ml-1">Account Status</label>
                            <select name="status" value={formData.status} onChange={handleInputChange} className="w-full pl-3 pr-4 py-3 text-sm font-semibold bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#0284C7]">
                                <option value="ACTIVE">ACTIVE</option>
                                <option value="INACTIVE">INACTIVE</option>
                                <option value="ON_LEAVE">ON LEAVE</option>
                            </select>
                ...
                <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden h-fit">
                    ...
                    <div className="p-6 space-y-5">
                        ...
                        <div>
                            <label className="text-[9px] font-bold uppercase text-slate-400 tracking-[0.2em] ml-1">Work Location</label>
                            <div className="flex gap-2">
                                <div className="bg-slate-100 text-slate-500 border border-slate-200 rounded-xl px-4 py-3 text-[11px] font-black flex items-center gap-2 uppercase shadow-sm">
                                    <LocationCity style={{ fontSize: 16 }} />
                                    {parentCityName}
                                </div>
                                <select name="subCityId" value={formData.subCityId} onChange={handleInputChange} className="flex-1 text-sm font-semibold bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-[#0284C7]">
                                    <option value="">-- Select Sub-City --</option>
                                    {subCities.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                                </select>
                            </div>
