...
export default function SubCities() {
    ...
    const fetchSubCities = async () => {
        try {
            ...
            showAlert('error', 'Failed to synchronize geographic data.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDeleteClick = (subCity) => {
        setDeleteConfig({ show: true, id: subCity.id, name: subCity.name });
    };
    ...
    const executeDelete = async () => {
        ...
        try {
            await adminApi.DELETE_SUB_CITY(id);
            showAlert('success', `Sub-city "${name}" removed.`);
            fetchSubCities();
        } catch (err) {
            showAlert('error', "Deletion failed: Item may be in use.");
        }
    };

    // 1. Filter Logic
    const filteredSubCities = useMemo(() => {
        return subCities.filter(sc =>
            (sc.name || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
            (sc.cityName || "").toLowerCase().includes(searchTerm.toLowerCase())
        );
    }, [subCities, searchTerm]);
        ...
        <div className="w-full space-y-4 animate-fadeIn px-2 pb-10 relative">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wider">Geographic Jurisdiction Settings</p>
                </div>
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50 border-b border-slate-100 text-slate-400 text-[9px] font-bold uppercase tracking-widest">
                        <tr>
                            <th className="px-6 py-3">Jurisdiction Name</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {loading ? (
                            <tr><td colSpan="3" className="px-6 py-10 text-center text-slate-400 text-xs italic">Syncing Geographic Registry...</td></tr>
                        ) : paginatedSubCities.length > 0 ? (
                            paginatedSubCities.map((sc) => (
                                <tr key={sc.id} className="hover:bg-slate-50/50 transition-colors group">
                                    <td className="px-6 py-3.5">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-sm font-semibold text-slate-700">{sc.name}</span>
                                        </div>
                                    </td>
                            ...
                            <tr><td colSpan="3" className="px-6 py-20 text-center text-slate-400 text-xs italic">No geographic data found matching "{searchTerm}"</td></tr>
                        )}
                    </tbody>
                </table>
