...
export default function Consultancies() {
    ...
    const fetchConsultancies = async () => {
        try {
            const res = await adminApi.GET_CONSULTANTS();
            setConsultancies(res.data?.data || res.data || res);
        } catch (err) {
    ...
    const executeDelete = async () => {
        ...
        try {
            await adminApi.DELETE_CONSULTANT(id);
        ...
        <div className="w-full space-y-4 animate-fadeIn px-2 pb-10 relative">
            ...
            <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm flex items-center">
                <div className="relative max-w-sm w-full">
                    ...
                    <input type="text" placeholder="Search Consultant..." className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs outline-none focus:border-[#0284C7] transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                <table className="w-full text-left">
                    <thead className="bg-slate-50/50 border-b border-slate-100 text-slate-400 text-[9px] font-bold uppercase tracking-widest">
                        <tr>
                            <th className="px-6 py-4">Consultant Name</th>
                            <th className="px-6 py-4">Consultant Category</th>
                            ...
                            <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm font-semibold text-slate-700">{c.consultantName}</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4">
                                    {c.document ? (
                                        <a href={`http://localhost:8080/api/admin/consultancy/download/${c.document}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-[#0284C7] hover:text-[#016da3] transition-colors">
