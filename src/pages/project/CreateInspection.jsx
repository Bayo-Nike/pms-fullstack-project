...
// Helper: Haversine Formula (Comparison logic)
...
export default function CreateInspection() {
    ...
    useEffect(() => {
        const resolveActual = async () => {
            ...
            // 1. Priority: Task Lat/Lng from Task Registry
            if (formData.inspectionLevel === 'TASK' && formData.taskId) {
                const task = allMyTasks.find(t => String(t.id) === String(formData.taskId));
                if (task?.latitude && task?.longitude) {
                    lat = parseFloat(task.latitude); lng = parseFloat(task.longitude);
                }
            }
            // 2. Fallback: Location API (Mapping to lat/lng from DTO)
            if ((!lat || !lng) && selectedProject?.locationIds?.length > 0) {
                try {
                    const res = await adminApi.GET_LOCATION(selectedProject.locationIds[0]);
                    const dto = res.data?.data || res.data;
                    if (dto?.lat && dto?.lng) { lat = parseFloat(dto.lat); lng = parseFloat(dto.lng); }
                } catch (e) { console.error(e); }
            }
            setActualCoords({ lat, lng });
        };
        resolveActual();
    }, [formData.taskId, formData.projectId, formData.inspectionLevel, allMyTasks, selectedProject]);

    useEffect(() => {
        if (formData.latitude && formData.longitude && actualCoords.lat && actualCoords.lng) {
            setProximityKm(getDistance(parseFloat(formData.latitude), parseFloat(formData.longitude), actualCoords.lat, actualCoords.lng));
        } else { setProximityKm(null); }
    }, [formData.latitude, formData.longitude, actualCoords]);
    ...
    const fetchLocation = async () => {
        ...
        } catch (err) { showAlert('warning', 'GPS Capture Failed: Ensure location services are active.'); }
    ...
    const triggerSave = () => {
        ...
        if (!projectId || !inspectionTypeId || !inspectionResult.trim() || !weatherCondition || !activeWorkers) {
            showAlert('error', 'Required verification fields (*) must be completed.'); return;
        }
        if (inspectionLevel === 'TASK' && !taskId) { showAlert('error', 'Specific Task Component must be defined.'); return; }
        setConfirmModal({
            show: true, title: isEdit ? 'Update Entry?' : 'Commit Log?',
            message: 'This record will be permanently saved with your ID and GPS coordinates. Proceed?'
        });
    };

    const executeSave = async () => {
        setConfirmModal({ ...confirmModal, show: false }); setSaving(true);
        try {
            const payload = {
        ...
        } catch (err) { showAlert('error', err.response?.data?.message || 'Transaction rejected.'); setSaving(false); }
    };
        ...
        <div className="w-full space-y-6 pb-12 px-6 bg-[#F8FAFC] animate-fadeIn text-slate-700">
                ...
                <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[40px] w-full max-w-sm shadow-2xl overflow-hidden animate-slideUp">
                        <div className="p-10 text-center space-y-6">
                            <div className="w-20 h-20 mx-auto rounded-full bg-sky-50 text-sky-500 flex items-center justify-center"><HelpOutline style={{ fontSize: 40 }} /></div>
                            <div className="space-y-2"><h3 className="text-xl font-black text-slate-800 uppercase">{confirmModal.title}</h3><p className="text-sm text-slate-500 leading-relaxed font-medium">{confirmModal.message}</p></div>
            ...
            <div className="flex items-center justify-between bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
                <div className="flex items-center gap-5">
                    ...
                    <div><h1 className="text-2xl font-black text-slate-900 leading-none">{isEdit ? 'Log Maintenance' : 'Quality Inspection'}</h1><p className="text-[10px] text-sky-600 mt-2 font-bold uppercase tracking-widest italic">Inspection Assessment</p></div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-slate-50 bg-slate-50/40 flex items-center gap-3"><Layers className="text-slate-400" fontSize="small" /><span className="text-[11px] font-black uppercase text-slate-500 tracking-widest">Inspection Scope</span></div>
                        <div className="p-8 space-y-6">
                            <div className="space-y-2">
                                ...
                                <div className="flex p-1.5 bg-slate-50 rounded-2xl border border-slate-100 gap-1">
                                    {['PROJECT', 'TASK'].map(lvl => (<button key={lvl} onClick={() => setFormData({ ...formData, inspectionLevel: lvl, taskId: '' })} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${formData.inspectionLevel === lvl ? 'bg-white text-[#0284C7] shadow-sm border border-slate-100' : 'text-slate-400'}`}>{lvl}</button>))}
                                </div>
                            </div>
                            <div className="space-y-2">
                                ...
                                <select value={formData.projectId} onChange={e => handleProjectChange(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-sm font-bold outline-none focus:border-sky-500 transition-all">
                                    <option value="">-- Choose Project --</option>{myProjects.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}
                                </select>
                            </div>
                            {formData.inspectionLevel === 'TASK' && (
                                <div className="space-y-2 animate-slideUp">
                                    ...
                                    <select value={formData.taskId} onChange={e => setFormData({ ...formData, taskId: e.target.value })} disabled={!formData.projectId} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-sm font-bold outline-none disabled:opacity-50">
                                        <option value="">-- Select Specific Task --</option>{filteredTasks.map(t => <option key={t.id} value={t.id}>{t.taskName}</option>)}
                                    </select>
                                </div>
                            )}
                            <div className="p-5 bg-slate-900 rounded-[24px] flex items-center gap-4 shadow-lg shadow-slate-200"><LocationCity className="text-sky-400" /><div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Regional Registry</p><p className="text-xs font-black text-white uppercase">{cityName}</p></div></div>
                        </div>
                    </div>

                    <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-slate-50 bg-slate-50/40 flex items-center gap-3"><EventNote className="text-slate-400" fontSize="small" /><span className="text-[11px] font-black uppercase text-slate-500 tracking-widest">Operational Metrics</span></div>
                        <div className="p-8 space-y-6">
                            <div className="space-y-2">
                                ...
                                <select value={formData.weatherCondition} onChange={(e) => setFormData({ ...formData, weatherCondition: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-sm font-bold outline-none">
                                    <option value="">-- Select Condition --</option>{['SUNNY', 'CLOUD', 'RAIN', 'WINDY', 'STORM', 'SNOW', 'UNKNOWN'].map(w => <option key={w} value={w}>{w}</option>)}
                                </select>
                            </div>
                            <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Active Workers on Site *</label><input type="number" value={formData.activeWorkers} onChange={(e) => setFormData({ ...formData, activeWorkers: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-sm font-bold outline-none" placeholder="0" /></div>
                            <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Inspection Date (As of Day)</label><input type="date" value={formData.inspectionDate} readOnly className="w-full bg-slate-100 border border-slate-100 rounded-2xl px-5 py-4 text-sm font-bold text-slate-400 cursor-not-allowed outline-none" /></div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden">
                            <div className="p-6 border-b border-slate-50 bg-slate-50/40 flex items-center justify-between">
                                <div className="flex items-center gap-3"><MyLocation className={fetchingGPS ? "animate-spin text-sky-500" : "text-slate-400"} fontSize="small" /><span className="text-[11px] font-black uppercase text-slate-500 tracking-widest">GPS Audit</span></div>
                                {!isEdit && (<button onClick={fetchLocation} className="text-[9px] font-black text-sky-600 uppercase hover:underline">Recalibrate</button>)}
                            </div>
                            <div className="p-8 space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Recorded Lat</label><input type="text" value={formData.latitude} readOnly className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 text-xs font-black text-slate-500 cursor-not-allowed outline-none" /></div>
                                    <div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Recorded Lng</label><input type="text" value={formData.longitude} readOnly className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 text-xs font-black text-slate-500 cursor-not-allowed outline-none" /></div>
                                </div>
                                {proximityKm !== null && proxStatus && (
                                    <div className={`flex items-center gap-3 p-4 rounded-2xl animate-fadeIn border ${proxStatus.bg} ${proxStatus.border}`}>
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${proxStatus.color} bg-white shadow-sm`}><Straighten fontSize="small" /></div>
                                        <div className="flex-1">
                                            <span className={`text-[8px] font-black px-1.5 py-0.5 rounded border uppercase ${proxStatus.color} ${proxStatus.border} bg-white`}>{proxStatus.label}</span>
                                            <p className={`text-xs font-black ${proxStatus.color} mt-1`}>
                                                {proximityKm < 1 ? `${(proximityKm * 1000).toFixed(0)} Meters` : `${proximityKm.toFixed(2)} KM`} from actual site
                                            </p>
                                        </div>
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center bg-white shadow-sm ${proxStatus.color}`}><CheckCircle style={{ fontSize: 16 }} /></div>
                                    </div>
                                )}
                                {formData.latitude && formData.longitude && (
                                    <a href={`https://www.google.com/maps/search/?api=1&query=${formData.latitude},${formData.longitude}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-3 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black text-sky-600 uppercase tracking-widest hover:bg-sky-50 transition-all group">
                                        <MapPin size={14} className="group-hover:animate-bounce" />View on Google Maps<ExternalLink size={12} className="opacity-50" />
                                    </a>
                                )}
                            </div>
                        </div>
