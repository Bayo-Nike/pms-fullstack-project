...
//     // Lifecycle Lock: Tasks can only be changed in INITIATED status
...
//             } catch (err) {
//                 setAlert({ show: true, type: 'error', message: 'Failed to synchronize record.' });
//             } finally {
//                 setLoading(false);
//             }
//         };
...
//     useEffect(() => {
//         if (isTaskModalOpen && project) {
//             setFilteredSuggestions(taskTypeRegistry.filter(t => t.projectType === project.projectType));
//         }
...
//         } catch (err) {
//             setAlert({ show: true, type: 'error', message: 'Task sync failed.' });
//         }
//     };
...
//             setAlert({ show: true, type: 'success', message: 'Task removed.' });
...
//                         <p className="text-sm text-slate-500 mt-2 leading-relaxed">Are you sure you want to remove <b>{deleteConfig.taskName}</b>? This cannot be undone.</p>
//                         <div className="flex gap-4 mt-10">
//                             <button onClick={() => setDeleteConfig({ show: false, id: null, taskName: '' })} className="flex-1 px-4 py-3 rounded-2xl border font-black uppercase text-[10px] tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
//                             <button onClick={executeDeleteTask} className="flex-1 px-4 py-3 bg-red-500 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-red-100 transition-all active:scale-95">Confirm Delete</button>
//                         </div>
//                     </div>
//                 </div>
//             )}

//             {/* Header: Project Info (Read Only) */}
...
//                     <div className="text-right border-r pr-8 border-slate-100"><p className="text-[9px] font-bold text-slate-400 uppercase">Initiation Phase</p><span className={`text-xs font-black uppercase ${project.status === 'STARTED' ? 'text-emerald-600' : 'text-[#0284C7]'}`}>{project.status}</span></div>
...
//                     <div className="text-right"><p className="text-[9px] font-bold text-slate-400 uppercase">Estimated Window</p><span className="text-xs font-black text-slate-700">{project.startDate || 'TBD'} &rarr; {project.endDate || 'TBD'}</span></div>
//                 </div>
//             </div>
...
//                         <div className="w-12 h-12 bg-[#0284C7] text-white rounded-2xl flex items-center justify-center shadow-lg">
//                             {canModifyTasks ? <Assignment /> : <Lock />}
//                         </div>
//                         <div>
//                             <h2 className="text-lg font-black text-slate-900 tracking-tight">Planned Components (Tasks)</h2>
//                             <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
//                                 {canModifyTasks ? 'Identify deliverables for this initiation' : 'Planning is locked for implemented projects'}
//                             </p>
//                         </div>
//                     </div>
//                     {canModifyTasks && (
//                         <button onClick={() => { setEditingTask(null); setTaskFormData({ taskName: '', taskCost: '', startDate: '', endDate: '', description: '', status: 'TO_DO', priority: 'LOW', weight: 0, latitude: '', longitude: '', locationIds: [], employeeIds: [] }); setSupportDocument(null); setIsTaskModalOpen(true); }} className="bg-[#0284C7] text-white px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">
//                             <Add /> New Task
//                         </button>
//                     )}
//                 </div>
...
//                             {tasks.length === 0 ? (
//                                 <tr><td colSpan="4" className="px-10 py-20 text-center text-slate-300 italic font-bold uppercase tracking-widest">No tasks defined for this initiation.</td></tr>
//                             ) : tasks.map((task) => (
...
//                                     <td className="px-6 py-5 text-center"><span className="text-[9px] font-black px-3 py-1.5 rounded-full border border-slate-200 bg-slate-50 text-slate-500 uppercase">{task.status}</span></td>
//                                     <td className="px-10 py-5 text-right">
//                                         <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
//                                             {/* Viewing icon (Always available) */}
...
//                                             {canModifyTasks && (
//                                                 <>
//                                                     <button onClick={() => openEditTask(task)} className="p-2 text-slate-400 hover:text-[#0284C7] transition-all"><Edit fontSize="small" /></button>
...
//                             </div>
//                             <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Scope Details</p><p className="text-xs text-slate-600 italic leading-relaxed">"{viewingTask.description || 'No additional scope details provided.'}"</p></div>
//                         </div>
//                         <div className="p-8 border-t bg-slate-50/30 flex justify-end"><button onClick={() => setViewingTask(null)} className="px-8 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest">Close View</button></div>
//                     </div>
//                 </div>
//             )}

//             {/* Task Creation/Editing Modal (Locked if canModifyTasks is false) */}
//             {isTaskModalOpen && (
...
//                             <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">{editingTask ? 'Edit Task Component' : 'New Planned Component'}</h3>
...
//                                         <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Component Title *</label>
//                                         <input list="init-task-suggestions" value={taskFormData.taskName} onChange={e => setTaskFormData({ ...taskFormData, taskName: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 outline-none focus:border-[#0284C7] text-sm font-bold" required placeholder="e.g. Land Clearing" />
//                                         <datalist id="init-task-suggestions">
//                                             {filteredSuggestions.map(t => <option key={t.id} value={t.name}>{t.projectType} Component</option>)}
//                                         </datalist>
//                                     </div>
//                                     <div className="grid grid-cols-2 gap-6">
//                                         <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Planned Weight (%)</label><input type="number" step="0.01" value={taskFormData.weight} onChange={e => setTaskFormData({ ...taskFormData, weight: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold" /></div>
//                                         <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Est. Cost</label><input type="number" value={taskFormData.taskCost} onChange={e => setTaskFormData({ ...taskFormData, taskCost: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold" /></div>
//                                     </div>
//                                     <div className="grid grid-cols-2 gap-6">
//                                         <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Target Start</label><input type="date" value={taskFormData.startDate} onChange={e => setTaskFormData({ ...taskFormData, startDate: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-bold text-sm" /></div>
//                                         <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Target End</label><input type="date" value={taskFormData.endDate} onChange={e => setTaskFormData({ ...taskFormData, endDate: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-bold text-sm" /></div>
//                                     </div>
//                                 </div>
//                                 <div className="space-y-6">
//                                     <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Justification / Detailed Scope</label><textarea rows="6" value={taskFormData.description} onChange={e => setTaskFormData({ ...taskFormData, description: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl px-6 py-5 text-sm font-medium resize-none focus:border-[#0284C7] outline-none" placeholder="Explain the importance of this task for the project initiation..."></textarea></div>
...
//                                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Attach Proposal Artifact</p>
//                                     </div>
...
//                                 </div>
//                             </div>
//                             <button type="submit" className="w-full bg-[#0284C7] text-white py-5 rounded-[24px] font-black uppercase text-xs shadow-xl shadow-sky-100 hover:bg-[#0369a1] active:scale-95 transition-all tracking-[0.2em]">
//                                 {editingTask ? 'Update Component Registry' : 'Commit Planned Component'}
//                             </button>
//                         </form>
//                     </div>
//                 </div>
...
//             setIsTaskModalOpen(false);
//             setAlert({ show: true, type: 'success', message: 'Task registry updated.' });
//         } catch (err) { setAlert({ show: true, type: 'error', message: 'Task sync failed.' }); }
//     };
...
//                         <div className="flex gap-4 mt-10">
//                             <button onClick={() => setDeleteConfig({ show: false, id: null, taskName: '' })} className="flex-1 px-4 py-3 rounded-2xl border font-black uppercase text-[10px] tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
//                             <button onClick={executeDeleteTask} className="flex-1 px-4 py-3 bg-red-500 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg active:scale-95 transition-all">Confirm Delete</button>
//                         </div>
//                     </div>
//                 </div>
//             )}
...
//                 </div>
//                 <div className="flex gap-8">
//                     <div className="text-right border-r pr-8 border-slate-100"><p className="text-[9px] font-bold text-slate-400 uppercase">Status</p><span className={`text-xs font-black uppercase ${project.status === 'STARTED' ? 'text-emerald-600' : 'text-[#0284C7]'}`}>{project.status}</span></div>
//                     <div className="text-right border-r pr-8 border-slate-100"><p className="text-[9px] font-bold text-slate-400 uppercase">Category</p><span className="text-xs font-black text-amber-600 uppercase">{project.category}</span></div>
//                     <div className="text-right"><p className="text-[9px] font-bold text-slate-400 uppercase">Estimated Window</p><span className="text-xs font-black text-slate-700">{project.startDate || 'TBD'} &rarr; {project.endDate || 'TBD'}</span></div>
//                 </div>
//             </div>
...
//                         <div><h2 className="text-lg font-black text-slate-900 tracking-tight">Planned Components (Tasks)</h2><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{canModifyTasks ? 'Identify deliverables for this initiation' : 'Planning is locked for implemented projects'}</p></div>
//                     </div>
//                     {canModifyTasks && (
...
//                                             {canModifyTasks && (
//                                                 <>
//                                                     <button onClick={() => { setEditingTask(task); setTaskFormData({ ...task }); setSupportDocument(null); setIsTaskModalOpen(true); }} className="p-2 text-slate-400 hover:text-[#0284C7] transition-all"><Edit fontSize="small" /></button>
//                                                     <button onClick={() => setDeleteConfig({ show: true, id: task.id, taskName: task.taskName })} className="p-2 text-slate-400 hover:text-red-500 transition-all"><Delete fontSize="small" /></button>
//                                                 </>
//                                             )}
//                                         </div>
...
//                                 <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Planned Cost</p><p className="text-sm font-bold text-slate-700">{project.currencyType} {viewingTask.taskCost?.toLocaleString()}</p></div>
//                             </div>
//                             <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Scope Details</p><p className="text-xs text-slate-600 italic leading-relaxed">"{viewingTask.description || 'No additional scope details.'}"</p></div>
//                         </div>
//                         <div className="p-8 border-t bg-slate-50/30 flex justify-end"><button onClick={() => setViewingTask(null)} className="px-8 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest">Close View</button></div>
//                     </div>
//                 </div>
...
//                             <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
//                                 <div className="space-y-6">
//                                     <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Component Title *</label>
//                                         <input list="init-task-suggestions" value={taskFormData.taskName} onChange={e => setTaskFormData({ ...taskFormData, taskName: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 outline-none focus:border-[#0284C7] text-sm font-bold" required />
//                                         <datalist id="init-task-suggestions">{filteredSuggestions.map(t => <option key={t.id} value={t.name}>{t.projectType} Component</option>)}</datalist>
//                                     </div>
//                                     <div className="grid grid-cols-2 gap-6">
//                                         <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Registry Status</label>
//                                             <select value={taskFormData.status} onChange={e => setTaskFormData({ ...taskFormData, status: e.target.value })} className="w-full bg-sky-50 border-2 border-sky-100 text-[#0284C7] rounded-2xl px-6 py-4 text-[11px] font-black uppercase outline-none">
...
//                                             </select>
//                                         </div>
//                                         <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Contribution (%)</label><input type="number" step="0.01" value={taskFormData.weight} onChange={e => setTaskFormData({ ...taskFormData, weight: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold" /></div>
//                                     </div>
//                                     <div className="grid grid-cols-2 gap-6">
//                                         <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Target Start</label><input type="date" value={taskFormData.startDate} onChange={e => setTaskFormData({ ...taskFormData, startDate: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-bold text-sm" /></div>
//                                         <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Target End</label><input type="date" value={taskFormData.endDate} onChange={e => setTaskFormData({ ...taskFormData, endDate: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-bold text-sm" /></div>
...
//                                 </div>
//                                 <div className="space-y-6">
//                                     <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Justification / Detailed Scope</label><textarea rows="5" value={taskFormData.description} onChange={e => setTaskFormData({ ...taskFormData, description: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl px-6 py-5 text-sm font-medium resize-none focus:border-[#0284C7] outline-none" placeholder="Execution plan details..."></textarea></div>
//                                     <div className="bg-slate-50 rounded-[28px] border-2 border-slate-100 border-dashed p-10 text-center relative group hover:bg-white transition-all cursor-pointer">
//                                         <input type="file" onChange={(e) => setSupportDocument(e.target.files[0])} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
//                                         <UploadFile className="text-slate-200 group-hover:text-[#0284C7] mb-2" style={{ fontSize: 40 }} />
//                                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Attach Artifact</p>
//                                     </div>
//                                     {supportDocument && <div className="flex items-center justify-between p-4 bg-sky-50 rounded-2xl border border-sky-100"><div className="flex items-center gap-3"><CloudDone className="text-[#0284C7]" /><span className="text-[11px] font-bold text-slate-700 truncate max-w-[200px]">{supportDocument.name}</span></div><Close onClick={() => setSupportDocument(null)} className="cursor-pointer text-slate-400" /></div>}
//                                 </div>
//                             </div>
//                             <button type="submit" className="w-full bg-[#0284C7] text-white py-5 rounded-[24px] font-black uppercase text-xs shadow-xl shadow-sky-100 hover:bg-[#0369a1] active:scale-95 transition-all tracking-[0.2em]">{editingTask ? 'Update Component Registry' : 'Commit Planned Component'}</button>
//                         </form>
//                     </div>
//                 </div>
//             )}
...
//     // Registry States (for standard component naming)
//     const [taskTypeRegistry, setTaskTypeRegistry] = useState([]);
//     const [filteredSuggestions, setFilteredSuggestions] = useState([]);
...
//     // Apply strict filtering for Task Type suggestions
//     useEffect(() => {
...
//                 t.taskTypeProjectStatus === 'INITIATED' // Only show Initiation-Phase Task Types
//             );
...
//                         <p className="text-sm text-slate-500 mt-2 leading-relaxed">Delete <b>{deleteConfig.taskName}</b> from initiation plan? This cannot be undone.</p>
//                         <div className="flex gap-4 mt-10">
...
//                                             {canModifyTasks && (
//                                                 <>
//                                                     <button onClick={() => { setEditingTask(task); setTaskFormData({ ...task }); setIsTaskModalOpen(true); }} className="p-2 text-slate-400 hover:text-[#0284C7] transition-all"><Edit fontSize="small" /></button>
//                                                     <button onClick={() => setDeleteConfig({ show: true, id: task.id, taskName: task.taskName })} className="p-2 text-slate-400 hover:text-red-500 transition-all"><Delete fontSize="small" /></button>
//                                                 </>
//                                             )}
//                                         </div>
...
//                                 <div className="space-y-6">
//                                     <div className="space-y-2">
//                                         <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Component Title (Initiation Registry) *</label>
//                                         <input list="init-task-suggestions" value={taskFormData.taskName} onChange={e => setTaskFormData({ ...taskFormData, taskName: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 outline-none focus:border-[#0284C7] text-sm font-bold" required placeholder="e.g. Feasibility Study" />
//                                         <datalist id="init-task-suggestions">
//                                             {filteredSuggestions.map(t => <option key={t.id} value={t.name}>{t.projectType} Initiation Task</option>)}
//                                         </datalist>
//                                     </div>
//                                     <div className="grid grid-cols-2 gap-6">
...
const ProjectInitiationDetails = () => {
    ...
    useEffect(() => {
        if (isTaskModalOpen && project && taskTypeRegistry.length > 0) {
            const filtered = taskTypeRegistry.filter(t =>
                t.projectType === project.projectType && // Constraint 1: Domain
                t.taskTypeProjectStatus === 'INITIATED'  // Constraint 2: Phase
            );
            setFilteredSuggestions(filtered);
        }
    }, [isTaskModalOpen, project, taskTypeRegistry]);
            ...
            {deleteConfig.show && (
                <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm px-4">
                    <div className="bg-white rounded-[32px] p-10 max-w-sm w-full text-center border shadow-2xl animate-scaleIn">
                        ...
                        <p className="text-sm text-slate-500 mt-2 leading-relaxed">Delete <b>{deleteConfig.taskName}</b> from initiation plan? This cannot be undone.</p>
                        <div className="flex gap-4 mt-10">
                            <button onClick={() => setDeleteConfig({ show: false, id: null, taskName: '' })} className="flex-1 px-4 py-3 rounded-2xl border font-black uppercase text-[10px] hover:bg-slate-50 transition-all">Cancel</button>
                            <button onClick={executeDeleteTask} className="flex-1 px-4 py-3 bg-red-500 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg">Confirm Delete</button>
                        </div>
                    </div>
                </div>
