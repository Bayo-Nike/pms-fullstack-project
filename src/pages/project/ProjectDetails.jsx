...
//         status: 'NEW', priority: 'MEDIUM', weight: 0,
...
//                 weight: parseFloat(taskFormData.weight)
//             };
...
//             setAlert({ show: true, type: 'success', message: 'Task committed.' });
//         } catch (err) { setAlert({ show: true, type: 'error', message: 'Failed.' }); }
//     };
...
//         } catch (err) { setAlert({ show: true, type: 'error', message: 'Error deleting.' }); }
//     };

//     if (loading) return <div className="p-20 text-center text-slate-400 animate-pulse">Establishing Context...</div>;
...
//                     <button onClick={() => navigate('/projects')} className="p-2 bg-slate-50 rounded-xl hover:bg-slate-100"><ArrowBack fontSize="small" /></button>
//                     <div><div className="flex items-center gap-2"><span className="text-[10px] font-black text-[#0284C7] bg-sky-50 px-1.5 py-0.5 rounded uppercase border border-sky-100">{project.projectCode}</span><h1 className="text-xl font-black text-slate-900">{project.title}</h1></div><p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1"><LocationOn style={{ fontSize: 12 }} /> {project.cityName} &bull; {project.subCityName}</p></div>
//                 </div>
...
//                         <div className="flex items-center justify-between mb-4 font-bold text-[10px] text-slate-400 uppercase tracking-widest"><Payments fontSize="small" /> Finance</div>
//                         <div className="grid grid-cols-2 gap-4">
//                             <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 text-center"><p className="text-[9px] font-bold text-slate-400 uppercase">Sum</p><p className="text-lg font-black text-slate-900">{project.currencyType} {project.budget.toLocaleString()}</p></div>
//                             <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 text-center"><p className="text-[9px] font-bold text-slate-400 uppercase">Spent</p><p className="text-lg font-black text-[#0284C7]">{project.currencyType} {project.budgetUsed.toLocaleString()}</p></div>
//                         </div>
//                     </div>
//                     <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm h-fit"><div className="flex items-center gap-2 mb-2 font-bold text-[10px] text-slate-400 uppercase tracking-widest"><Info fontSize="small" /> Dossier</div><p className="text-xs text-slate-500 italic line-clamp-3">{project.description || 'N/A'}</p></div>
//                 </div>
//                 <div className="lg:col-span-4 space-y-4">
//                     <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm space-y-4">
//                         <div className="flex items-center gap-2 font-bold text-[10px] text-slate-400 uppercase tracking-widest"><Engineering fontSize="small" /> Stakeholders</div>
//                         <div className="space-y-2">
//                             <div className="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-100"><Badge className="text-sky-50" style={{ fontSize: 16, color: '#0284C7' }} /><div><p className="text-[8px] font-bold text-slate-400 uppercase">PM</p><p className="text-xs font-bold text-slate-700">{project.projectManagerName}</p></div></div>
//                             <div className="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-100"><Work className="text-amber-50" style={{ fontSize: 16, color: '#FBAF1E' }} /><div><p className="text-[8px] font-bold text-slate-400 uppercase">Cont</p><p className="text-xs font-bold text-slate-700">{project.contractorName}</p></div></div>
//                         </div>
//                     </div>
//                     <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm h-fit"><div className="flex items-center gap-2 mb-3 font-bold text-[10px] text-slate-400 uppercase tracking-widest"><TrendingUp fontSize="small" /> Team</div><div className="flex flex-wrap gap-1">{project.employeeNames?.map((n, i) => (<span key={i} className="px-1.5 py-0.5 bg-slate-100 text-[8px] font-bold text-slate-500 rounded uppercase border border-slate-200">{n}</span>))}</div></div>
//                 </div>
//             </div>
...
//                     <button onClick={() => { setEditingTask(null); setTaskFormData({ taskName: '', status: 'NEW', priority: 'MEDIUM', weight: 0, startDate: '', endDate: '', description: '', latitude: '', longitude: '', locationIds: [], employeeIds: [] }); setIsTaskModalOpen(true); }} className="bg-[#0284C7] text-white px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg flex items-center gap-2"><Add style={{ fontSize: 18 }} /> Add Task</button>
//                 </div>
...
//                         <tbody className="divide-y divide-slate-50">{tasks.length === 0 ? (<tr><td colSpan="4" className="px-8 py-10 text-center text-slate-400 text-xs italic">No records.</td></tr>) : tasks.map((task) => (
//                             <tr key={task.id} className="hover:bg-slate-50/50 transition-colors group">
//                                 <td className="px-8 py-4"><div className="flex flex-col"><span className="text-sm font-bold text-slate-700">{task.taskName}</span><div className="flex items-center gap-2 mt-1"><span className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1"><Schedule style={{ fontSize: 10 }} /> {new Date(task.startDate).toLocaleDateString()}</span><span className="text-[9px] font-bold text-slate-300 uppercase">&rarr;</span><span className="text-[9px] font-bold text-slate-400 uppercase">{new Date(task.endDate).toLocaleDateString()}</span></div></div></td>
...
//                                 <td className="px-6 py-4 text-center"><span className={`text-[9px] font-black px-2 py-1 rounded border uppercase tracking-tighter ${task.status === 'COMPLETED' ? 'bg-green-50 text-green-700' : 'bg-sky-50 text-sky-700'}`}>{task.status}</span></td>
...
//                         <div className="p-6 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between"><h3 className="font-black text-slate-800 uppercase tracking-tight">{editingTask ? 'Edit Task' : 'Register Task'}</h3><button onClick={() => setIsTaskModalOpen(false)} className="p-1.5 hover:bg-white rounded-full text-slate-400"><Close /></button></div>
...
//                                         <div className="flex items-center gap-2 text-slate-400 font-bold text-[9px] uppercase tracking-widest"><Explore style={{ fontSize: 16 }} /> Geo-Coordinates</div>
...
//                                         <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Tagged Locations (Multi-Select)</label>
//                                         <select onChange={(e) => { const v = Number(e.target.value); if (v && !taskFormData.locationIds.includes(v)) setTaskFormData(p => ({ ...p, locationIds: [...p.locationIds, v] })); }} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs font-bold uppercase"><option value="">-- Project Sites --</option>{projectSites.filter(s => !taskFormData.locationIds.includes(s.id)).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
...
//                                         <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Assigned Personnel (Multi-Select)</label>
//                                         <select onChange={(e) => { const v = Number(e.target.value); if (v && !taskFormData.employeeIds.includes(v)) setTaskFormData(p => ({ ...p, employeeIds: [...p.employeeIds, v] })); }} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs font-bold uppercase"><option value="">-- Project Team --</option>{projectStaff.filter(s => !taskFormData.employeeIds.includes(s.id)).map(s => <option key={s.id} value={s.id}>{s.fullName}</option>)}</select>
...
//                                     </div>
//                                     <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Task Scope Details</label><textarea rows="4" value={taskFormData.description} onChange={e => setTaskFormData({ ...taskFormData, description: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-medium resize-none"></textarea></div>
//                                 </div>
//                             </div>
//                             <button type="submit" className="w-full bg-[#0284C7] text-white py-4 rounded-2xl font-bold uppercase tracking-[0.2em] text-xs shadow-xl shadow-sky-100 hover:bg-[#016da3] transition-all mt-4">Confirm Task Registry</button>
//                         </form>
//                     </div>
//                 </div>
//             )}
...
//     // Task Form State (Matches CreateTaskRequestDTO)
//     const [taskFormData, setTaskFormData] = useState({
//         taskName: '',
//         startDate: '',
//         endDate: '',
...
//                 setTasks(tRes.data.data || []);
//             } catch (err) {
//                 setAlert({ show: true, type: 'error', message: 'Failed to load project details.' });
//             } finally {
//                 setLoading(false);
//             }
//         };
...
//     // Helpers to filter Task Assignments by Project Scope
//     const projectStaff = useMemo(() => {
//         if (!project) return [];
//         return project.employeeIds.map((eid, i) => ({
//             id: eid,
//             fullName: project.employeeNames[i]
//         })).filter(e => e.fullName.toLowerCase().includes(teamSearch.toLowerCase()));
//     }, [project, teamSearch]);
...
//         // --- DATA SANITIZATION FOR DTO (Fixes 400 Error) ---
//         const payload = {
...
//             startDate: taskFormData.startDate || null, // Cannot be empty string
//             endDate: taskFormData.endDate || null,     // Cannot be empty string
//             description: taskFormData.description || null,
//             status: taskFormData.status,         // e.g. "NEW"
//             priority: taskFormData.priority,     // e.g. "MEDIUM"
//             latitude: taskFormData.latitude ? parseFloat(taskFormData.latitude) : null,
//             longitude: taskFormData.longitude ? parseFloat(taskFormData.longitude) : null,
...
//         } catch (err) {
//             const errorMsg = err.response?.data?.message || 'Transaction failed. Check numeric constraints.';
...
//     if (loading) return <div className="p-20 text-center animate-pulse italic text-slate-400">Loading PMS Data...</div>;
//     if (!project) return <div className="p-20 text-center text-slate-300 font-bold uppercase">Record Not Found</div>;
...
//             <div className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex items-center justify-between">
//                 <div className="flex items-center gap-4">
//                     <button onClick={() => navigate('/projects')} className="p-2 bg-slate-50 border rounded-xl hover:bg-slate-100"><ArrowBack fontSize="small" /></button>
//                     <div>
...
//             {/* ROW 1 & 2: Project Summary */}
//             <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
//                 <div className="lg:col-span-8 space-y-4">
//                     <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm">
//                         <div className="flex items-center justify-between mb-4 font-bold text-[10px] text-slate-400 uppercase tracking-widest"><Payments fontSize="small" /> Finance Summary</div>
//                         <div className="grid grid-cols-2 gap-4">
...
//                         </div>
//                     </div>
//                     <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm h-fit"><div className="flex items-center gap-2 mb-2 font-bold text-[10px] text-slate-400 uppercase tracking-widest"><Info fontSize="small" /> Dossier</div><p className="text-xs text-slate-500 italic line-clamp-2">{project.description || 'N/A'}</p></div>
//                 </div>
//                 <div className="lg:col-span-4 space-y-4">
//                     <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm space-y-4">
//                         <div className="flex items-center gap-2 font-bold text-[10px] text-slate-400 uppercase tracking-widest"><Engineering fontSize="small" /> Stakeholders</div>
...
//             <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden">
//                 <div className="p-6 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
//                     <div className="flex items-center gap-3"><div className="w-10 h-10 bg-[#0284C7] text-white rounded-2xl flex items-center justify-center shadow-lg shadow-sky-100"><Assignment /></div><div><h2 className="text-lg font-bold text-slate-900 leading-none">Task Registry</h2><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Lifecycle Tracking</p></div></div>
//                     <button onClick={() => { setEditingTask(null); setTaskFormData({ taskName: '', startDate: '', endDate: '', description: '', status: 'NEW', priority: 'MEDIUM', weight: 0, latitude: '', longitude: '', locationIds: [], employeeIds: [] }); setIsTaskModalOpen(true); }} className="bg-[#0284C7] text-white px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg flex items-center gap-2"><Add style={{ fontSize: 18 }} /> New Task</button>
//                 </div>
//                 <div className="overflow-x-auto">
//                     <table className="w-full text-left"><thead className="bg-slate-50 text-[9px] font-bold text-slate-400 uppercase tracking-widest"><tr><th className="px-8 py-4">Task Component</th><th className="px-6 py-4 text-center">Weight</th><th className="px-6 py-4 text-center">Status</th><th className="px-8 py-4 text-right">Actions</th></tr></thead>
//                         <tbody className="divide-y divide-slate-50">{tasks.length === 0 ? (<tr><td colSpan="4" className="px-8 py-10 text-center text-slate-400 text-xs italic font-bold">No tasks defined.</td></tr>) : tasks.map((task) => (
//                             <tr key={task.id} className="hover:bg-slate-50/50 transition-colors group">
//                                 <td className="px-8 py-4"><div className="flex flex-col"><span className="text-sm font-bold text-slate-700">{task.taskName}</span><div className="flex items-center gap-2 mt-1"><span className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1"><Schedule style={{ fontSize: 10 }} /> {task.startDate} &rarr; {task.endDate}</span></div></div></td>
//                                 <td className="px-6 py-4 text-center"><span className="text-xs font-black text-slate-900 bg-amber-50 px-2 py-1 rounded-lg border border-amber-100">{task.weight}%</span></td>
//                                 <td className="px-6 py-4 text-center"><span className={`text-[9px] font-black px-2 py-1 rounded border uppercase tracking-tighter ${task.status === 'COMPLETED' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-sky-50 text-sky-700 border-sky-100'}`}>{task.status}</span></td>
//                                 <td className="px-8 py-4 text-right"><div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => { setEditingTask(task); setTaskFormData({ ...task }); setIsTaskModalOpen(true); }} className="p-1.5 text-slate-400 hover:text-[#0284C7] transition-all"><Edit style={{ fontSize: 18 }} /></button><button onClick={() => deleteTask(task.id)} className="p-1.5 text-slate-400 hover:text-red-500 transition-all"><Delete style={{ fontSize: 18 }} /></button></div></td>
//                             </tr>))}
//                         </tbody>
//                     </table>
...
//                 <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
//                     <div className="bg-white rounded-[32px] shadow-2xl border border-slate-100 w-full max-w-4xl overflow-hidden max-h-[95vh] flex flex-col">
//                         <div className="p-6 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between"><h3 className="font-black text-slate-800 uppercase tracking-tight">{editingTask ? 'Modify Task' : 'New Task Initiation'}</h3><button onClick={() => setIsTaskModalOpen(false)} className="p-1.5 hover:bg-white rounded-full text-slate-400"><Close /></button></div>
//                         <form onSubmit={handleTaskAction} className="p-8 overflow-y-auto space-y-6 no-scrollbar">
//                             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
//                                 <div className="space-y-4">
//                                     <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Task Title *</label><input value={taskFormData.taskName} onChange={e => setTaskFormData({ ...taskFormData, taskName: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 outline-none focus:border-[#0284C7] text-sm font-bold" required /></div>
//                                     <div className="grid grid-cols-2 gap-4">
//                                         <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Weight Contribution (%)</label><input type="number" step="0.01" value={taskFormData.weight} onChange={e => setTaskFormData({ ...taskFormData, weight: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 outline-none" /></div>
//                                         <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Priority</label><select value={taskFormData.priority} onChange={e => setTaskFormData({ ...taskFormData, priority: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs font-bold uppercase"><option value="LOW">Low</option><option value="MEDIUM">Medium</option><option value="HIGH">High</option></select></div>
//                                     </div>
//                                     <div className="grid grid-cols-2 gap-4">
//                                         <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Start Date</label><input type="date" value={taskFormData.startDate} onChange={e => setTaskFormData({ ...taskFormData, startDate: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 font-bold text-sm" /></div>
...
//                                 <div className="space-y-4">
//                                     <div className="space-y-1.5" ref={dropdownRef}>
//                                         <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Assigned Sites (Multi-Select)</label>
//                                         <select onChange={(e) => { const v = Number(e.target.value); if (v && !taskFormData.locationIds.includes(v)) setTaskFormData(p => ({ ...p, locationIds: [...p.locationIds, v] })); }} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs font-bold uppercase"><option value="">-- Tag Locations --</option>{projectSites.filter(s => !taskFormData.locationIds.includes(s.id)).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
//                                         <div className="flex flex-wrap gap-1.5 mt-2">{taskFormData.locationIds.map(lid => { const s = projectSites.find(x => x.id === lid); return <div key={lid} className="flex items-center gap-2 bg-slate-800 text-white pl-2 pr-1 py-1 rounded-lg text-[8px] font-black uppercase tracking-tighter">{s?.name}<Close onClick={() => setTaskFormData(p => ({ ...p, locationIds: p.locationIds.filter(i => i !== lid) }))} style={{ fontSize: 12 }} className="cursor-pointer" /></div>; })}</div>
//                                     </div>
//                                     <div className="space-y-1.5">
//                                         <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Team Assignment (Multi-Select)</label>
//                                         <select onChange={(e) => { const v = Number(e.target.value); if (v && !taskFormData.employeeIds.includes(v)) setTaskFormData(p => ({ ...p, employeeIds: [...p.employeeIds, v] })); }} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs font-bold uppercase"><option value="">-- Add Personnel --</option>{projectStaff.filter(s => !taskFormData.employeeIds.includes(s.id)).map(s => <option key={s.id} value={s.id}>{s.fullName}</option>)}</select>
//                                         <div className="flex flex-wrap gap-1.5 mt-2">{taskFormData.employeeIds.map(eid => { const s = projectStaff.find(x => x.id === eid); return <div key={eid} className="flex items-center gap-2 bg-[#0284C7] text-white pl-2 pr-1 py-1 rounded-lg text-[8px] font-black uppercase tracking-tighter">{s?.fullName}<Close onClick={() => setTaskFormData(p => ({ ...p, employeeIds: p.employeeIds.filter(i => i !== eid) }))} style={{ fontSize: 12 }} className="cursor-pointer" /></div>; })}</div>
//                                     </div>
//                                     <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Task Scope Details</label><textarea rows="5" value={taskFormData.description} onChange={e => setTaskFormData({ ...taskFormData, description: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-medium resize-none"></textarea></div>
//                                 </div>
//                             </div>
//                             <button type="submit" className="w-full bg-[#0284C7] text-white py-4 rounded-2xl font-bold uppercase tracking-[0.2em] text-xs shadow-xl shadow-sky-100 hover:bg-[#016da3] transition-all">Establish Task Configuration</button>
//                         </form>
//                     </div>
//                 </div>
//             )}
