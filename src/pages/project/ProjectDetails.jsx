...
const ProjectDetails = () => {
    ...
    const handleTaskAction = async (e) => {
        ...
        try {
            ...
            setModalError(err?.response?.data?.message || "Submission failed. Please check your inputs.");
        }
    };
                        ...
                        <form onSubmit={handleTaskAction} className="p-8 overflow-y-auto space-y-6 no-scrollbar">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Feedback/Remark</label><textarea rows="3" value={taskFormData.remark} onChange={e => setTaskFormData({ ...taskFormData, remark: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-medium resize-none focus:border-[#0284C7] outline-none" placeholder="Task Feedback/Remark..." /></div>
                                    <div className="bg-white rounded-2xl border p-3 space-y-2"><div className="flex items-center gap-1 text-[11px] font-bold uppercase text-slate-400"><Description fontSize="small" /> Artifact (Preserved)</div><div className="border-2 border-dashed rounded-[28px] p-3 text-center relative cursor-pointer group bg-slate-50/20"><input type="file" onChange={(e) => setSupportDocument(e.target.files[0])} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" /><UploadFile className="text-slate-100 group-hover:text-[#0284C7] mb-2" style={{ fontSize: 32 }} /><p className="text-[10px] font-bold text-slate-500 group-hover:text-[#0284C7] uppercase font-black">Upload Proof</p></div>{(supportDocument || existingFile) && (<div className="flex items-center gap-2 p-2 rounded-xl border bg-sky-50/30 border-sky-100"><Description className="text-[#0284C7]" /><div className="flex-1 min-w-0"><p className="text-[10px] font-bold truncate text-slate-700">{supportDocument ? supportDocument.name : existingFile}</p></div>{supportDocument && <Close onClick={() => setSupportDocument(null)} className="cursor-pointer text-slate-200" style={{ fontSize: 14 }} />}</div>)}</div>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 z-[1600] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white rounded-[40px] shadow-2xl border w-full max-w-2xl overflow-hidden flex flex-col">
                        ...
                        <div className="p-10 space-y-6">
                            <div className="flex justify-between items-start"><div className="space-y-1"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Task Name</p><p className="text-xl font-black text-slate-800">{viewingTask.taskName}</p></div><span className={`text-[10px] font-black px-4 py-2 rounded-xl border uppercase ${getTaskStatusStyle(viewingTask.status)}`}>{viewingTask.status.replace(/_/g, ' ')}</span></div>
                            ...
                            <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Description</p><p className="text-xs text-slate-600 italic leading-relaxed">"{viewingTask.description || 'No additional description provided.'}"</p></div>
                            <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Feedback/Remark</p><p className="text-xs text-slate-600 italic leading-relaxed">"{viewingTask.remark || 'No additional remark provided.'}"</p></div>
                        </div>
