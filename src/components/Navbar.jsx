...
//         // Only update unreadCount on initial load or background refresh
//         // For 1000+ items, you should ideally have a separate "count" API
//         // But for now, we'll estimate from the current list or totalElements
//         const count = content.filter(n => checkIsUnread(n)).length;
//         setUnreadCount(count);
//       }

//       setPage(pageNum);
//       setHasMore(!pageData?.last); // "last" is a boolean provided by Spring Page
//     } catch (err) {
//       // Silent error
//     } finally {
...
//   // Background Polling: Only refreshes the first page silently
//   useEffect(() => {
//     fetchData(0, false);
...
//     // Trigger next page when hitting bottom
...
//             n.id === notif.id ? { ...n, read: true, isRead: true, seen: true } : n
//           ));
//           setSelectedNotif(prev => ({ ...prev, read: true, isRead: true, seen: true }));
//         }
//       } catch (err) {
//       } finally {
...
//           <span className="text-xl md:text-2xl font-black tracking-tighter italic">SCCO <span className="text-[#FBAF1E]">PMS</span></span>
//         </div>
//       </div>
...
//           <button
//             onClick={() => {
//               if (!showNotifDropdown) { setSelectedNotif(null); fetchData(0, false); }
...
//           {showNotifDropdown && (
//             <div className="absolute right-[-60px] md:right-0 mt-3 w-[calc(100vw-32px)] sm:w-96 bg-white rounded-[24px] shadow-2xl border border-slate-100 overflow-hidden animate-fadeIn z-50">
//               <div className="px-6 py-4 border-b border-slate-50 flex items-center gap-3 bg-slate-50/50">
//                 {selectedNotif && (
//                   <button onClick={() => setSelectedNotif(null)} className="p-1.5 hover:bg-slate-200 rounded-full text-slate-500 transition-all" type="button"><ArrowBack style={{ fontSize: 18 }} /></button>
//                 )}
//                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-[3px]">
//                   {selectedNotif ? "Detail View" : `Notifications (${unreadCount})`}
//                 </span>
//               </div>

//               <div
//                 onScroll={!selectedNotif ? handleScroll : undefined}
//                 className="max-h-[420px] overflow-y-auto no-scrollbar relative min-h-[250px] flex flex-col"
//               >
//                 {selectedNotif ? (
...
//                       onClick={() => { setShowNotifDropdown(false); navigate(selectedNotif.notificationUrl); }}
//                       className={`mt-auto w-full py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 ${isMarkingRead ? 'bg-slate-100 text-slate-300' : 'bg-[#0284C7] text-white hover:bg-[#0369a1]'
//                         }`}
//                       type="button"
//                     >
...
//                         <DoneAll className="text-emerald-500/10 mb-2" style={{ fontSize: 64 }} />
//                         <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Everything up to date</p>
//                       </div>
//                     ) : (
//                       <>
//                         {notifications.map((notif) => {
//                           const unread = checkIsUnread(notif);
//                           return (
//                             <div
//                               key={notif.id}
//                               onClick={() => handleItemClick(notif)}
//                               className="p-5 border-b border-slate-50 hover:bg-slate-50/80 cursor-pointer transition-all flex gap-4 group"
//                             >
//                               <div className="mt-1.5 flex-shrink-0">
//                                 <Circle className={unread ? "text-[#FBAF1E]" : "text-slate-100"} style={{ fontSize: 10 }} />
//                               </div>
//                               <div className="flex-1">
//                                 <p className={`text-[12px] line-clamp-2 ${unread ? 'text-slate-800 font-black' : 'text-slate-400 font-medium'}`}>
//                                   {notif.message}
//                                 </p>
//                               </div>
...
//                         {isLoadingMore && (
//                           <div className="p-4 text-center text-[9px] font-black text-slate-400 animate-pulse uppercase tracking-widest">Loading more...</div>
//                         )}
//                       </>
//                     )}
//                   </>
...
//           {showDropdown && (
//             <div className="absolute right-0 mt-3 w-64 bg-white rounded-[24px] shadow-2xl border border-slate-100 animate-fadeIn py-2 z-50 overflow-hidden">
...
//               </div>
//               <button onClick={() => { logout(); navigate("/login"); }} className="w-full flex items-center gap-3 px-5 py-3 text-red-500 hover:bg-red-50 text-xs font-black uppercase tracking-widest transition-colors" type="button"><Logout style={{ fontSize: 18 }} /> Sign Out</button>
//             </div>
//           )}
//         </div>
//       </div>
...
//   Notifications, Menu as BurgerMenu, Logout, KeyboardArrowDown,
//   Circle, DoneAll, NavigateNext, ArrowBack, OpenInNew,
//   Lock, Visibility, VisibilityOff, Close, VpnKey, CheckCircle
// } from "@mui/icons-material";
// import { useAuth } from "../context/AuthContext";
// import notificationApi from "../api/modules/notification";
// import authApi from "../api/modules/auth"; // Ensure this points to your authApi
...
//       });
//       if (res?.data?.success) {
//         setPassStatus({ type: "success", msg: "Success! Logging out..." });
//         setTimeout(() => {
//           logout();
//           navigate("/login");
//         }, 2000);
//       }
//     } catch (err) {
//       setPassStatus({ type: "error", msg: err.response?.data?.message || "Error changing password" });
...
//                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-[3px]">{selectedNotif ? "Detail View" : `Notifications (${unreadCount})`}</span>
//                 </div>
...
//       {/* --- Change Password Modal (Tailwind Dialog) --- */}
//       {openChangePass && (
...
//                 <div>
//                   <h3 className="font-black text-sm uppercase tracking-widest">Security Update</h3>
//                   <p className="text-[9px] font-medium opacity-70">Update your access credentials</p>
//                 </div>
//               </div>
...
//               {passStatus.msg && (
//                 <div className={`p-4 rounded-2xl flex items-center gap-3 text-[10px] font-black uppercase tracking-wider ${passStatus.type === "success" ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}>
...
//                     <input
//                       required
//                       type={showPass[field.replace('Password', '')] ? "text" : "password"}
//                       className="w-full px-5 py-3.5 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-[#0284C7] focus:bg-white outline-none text-sm font-bold transition-all"
//                       value={passData[field]}
//                       onChange={(e) => setPassData({ ...passData, [field]: e.target.value })}
//                     />
//                     <button
//                       type="button"
//                       onClick={() => setShowPass(p => ({ ...p, [field.replace('Password', '')]: !p[field.replace('Password', '')] }))}
//                       className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500"
//                     >
//                       {showPass[field.replace('Password', '')] ? <VisibilityOff style={{ fontSize: 20 }} /> : <Visibility style={{ fontSize: 20 }} />}
//                     </button>
//                   </div>
//                 </div>
//               ))}
...
//                 type="submit"
//                 className="w-full py-4 bg-[#0284C7] text-white rounded-2xl font-black uppercase tracking-[2px] text-xs shadow-lg hover:bg-[#0369a1] active:scale-95 transition-all disabled:opacity-50 mt-4"
//               >
